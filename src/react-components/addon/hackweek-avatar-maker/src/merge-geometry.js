...
function mergeSourceAttributes({ sourceAttributes }) {
  console.log("mergeSourceAttributes",sourceAttributes);
  ...
  Array.from(propertyNames.keys()).map((name) => {
    console.log("attribute,",name,allSourceAttributes.map((sourceAttributes) => sourceAttributes[name]).flat());
...
export function mergeGeometry({ meshes }) {
  console.log("merge geometry,",meshes);
  const source = {
    meshes,
